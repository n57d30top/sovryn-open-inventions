...
The historical Product FundClass remains preserved in copied Product artifacts, but the public active class is `not_discovery_scored_raw_reproduction_failed`. This repair does not change the Product claim or Product Gate artifact; it records that the public package is not externally review-ready because exact raw-data scientific reproduction failed.
...
The public package records a Product-generated Matbench/materials candidate package where `matbench_descriptor_transfer_significance_generator-output-01` has measured outcome `0.72`, residual magnitude `0.21`, and three recorded simple controls. Those values are exactly replayable from Product runtime formulas, but the raw scientific reproduction repair classifies them as runtime-derived deterministic generator scalars rather than independently reproduced materials-science outputs.

## What Is Shown

- The historical Product Fund Gate passed for candidate `DISCOVERY-LIFT-INSIGHT-HARD-GEN-MATBENCH-DESCRIPTOR-TRANSFER-SIGNIFICAN-74933C45D6DB`.
- The public package preserves the Product claim, FundClass, Fund Gate result, and no-overclaim statements.
...
- The standalone script currently classifies reproduction as `raw_scientific_reproduction_failed_product_values_runtime_derived`; it exactly replays Product runtime scalars, but the raw-data scientific residual failed reproduction.
...
## What Would Falsify Or Downgrade The Candidate

- Composition, formula-size, target-family, split-leakage, or shuffled-target controls absorb the residual under independent reviewer implementation.
- A matched negative/control slice reproduces the same effect.
- The public Matbench raw data cannot reproduce the recorded target outcome or residual direction.
- The holdout path is not independent of the claim-forming slice.
- The result is already known, source-family documented, or ordinary benchmark behavior.
- The formal-object/package-generation wording is found to contaminate the Matbench evidence path.
